	...
	elseif($numCharecter=12){
		...
		elseif(mysql_num_rows($checkIP) > 0){
			...
			if(preg_match("/^[a-zA-Z0-9]+$/", $_POST['name_add']) == 0){
				...
				$query_add = "INSERT INTO `dhcp`.`class5` (`hw`, `zone`, `name`, `ip`, `expire`) VALUES ('".$result_mac."','".$_POST["zone_add"]."', '".$_POST["name_add"]."','".$_POST["ip_add"]."','".$_POST["time_add"]."')";

				mysql_query($query_add) or die(mysql_error());
				header('Location: config_dhcp_class5.php');
