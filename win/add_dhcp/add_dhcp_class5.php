...
elseif($date12 < 0){
	...
	elseif($numCharecter=12){
		...
		$mac_query = "SELECT * FROM `class5` WHERE `hw` = '".$result_mac."'";
		$checkMac = mysql_query("$mac_query");

		$result_zone = trim($_POST["zone_add"]);
		$zone_query = "SELECT * FROM `class5` WHERE `hostname` = '".$result_zone."'";
		$checkZone = mysql_query("$zone_query");
		
		$result_name = trim($_POST["name_add"]);
		$name_query = "SELECT * FROM `class5` WHERE `name` = '".$result_name."'";
		$checkName = mysql_query("$name_query");
		
		$result_ip = trim($_POST["ip_add"]);
		$ip_query = "SELECT * FROM `class5` WHERE `ip` = '".$result_ip."'";
		...
		elseif(mysql_num_rows($checkIP) > 0){
			echo "IP address exists already.";
                        ...
                        $rangeStart = ip2long("10.40.0.1");
                        $rangeEnd = ip2long("10.49.255.255");
                        ...
                        elseif($rangeIp < $rangeStart or $rangeIp > $rangeEnd){
                                echo "Change Ip Address between 10.40.0.2 - 10.49.255.254";
                        }

			else{
				$query_add = "INSERT INTO `dhcp`.`class5` (`hw`, `name`, `ip`, `expire`) VALUES ('".$result_mac."','".$_POST["name_add"]."','".$_POST["ip_add"]."','".$_POST["time_add"]."')";
