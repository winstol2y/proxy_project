...
elseif($date12 < 0){
	...
	elseif($numCharecter=12){
		...
		$mac_query = "SELECT * FROM `class3` WHERE `hw` = '".$result_mac."'";
		$checkMac = mysql_query("$mac_query");

		$result_zone = trim($_POST["zone_add"]);
		$zone_query = "SELECT * FROM `class3` WHERE `hostname` = '".$result_zone."'";
		...
		elseif(mysql_num_rows($checkIP) > 0){
			echo "IP address exists already.";
                        ...
                        $rangeStart = ip2long("10.20.0.1");
                        $rangeEnd = ip2long("10.29.255.255");
                        ...
                        elseif($rangeIp < $rangeStart or $rangeIp > $rangeEnd){
                                echo "Change Ip Address between 10.20.0.2 - 10.29.255.254";
                        }
				...
				header('Location: dhcp_class3.php');
