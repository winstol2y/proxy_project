<?php
$data = file_get_contents('/var/log/squid3/access.log.1');
$dataLogArr = explode ( "\n", $data );
...
$t1 = date("d-m-Y", strtotime( '-1 days' ) ); //time now
...
$nameTable = "INSERT INTO `squidLogDay`.`tableName` (tableName) VALUES ('$t1')";
...
for($i = 0; $i < $lengthData; $i++){
	...
	$insertDataTable = "INSERT INTO `squidLogDay`.`$t1` (dateTime,timeUse,ipClient,resultCode,byte,method,url,hCode,type) VALUES ('$time','$data[1]','$data[2]','$data[3]','$data[4]','$data[5]','$data[6]','$data[8]','$data[9]')";
