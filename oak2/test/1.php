...
while($tableName = mysql_fetch_assoc($queryName)){
	...
	for($count = 0; $count < count($duplicateUrl); $count++){
		...
		$selectCountUrl  = 'SELECT count(url),sum(byte),sum(timeUse) FROM `squidLogDaily`.`'.$tableName["tableName"].'` WHERE `url` LIKE "%'.$duplicateUrl["$count"].'%"';
		...
		while($countUrl = mysql_fetch_assoc($queryCountUrl)){
			...
			echo "----->".$countUrl["sum(timeUse)"]." byte";
