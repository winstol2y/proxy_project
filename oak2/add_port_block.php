<?php
	...
	if(empty($_POST["port_add"]))
	{
		$statusQuery2 = FALSE;
		$texttoalert = $texttoalert.'Check Port \n';
	}
	else
	{
		$porttoDB = preg_replace('/[^\w\*]+|[A-Z]+|[a-z]+/', ' ', $_POST["port_add"]);
		$porttoDB = explode(' ', $porttoDB);
		$porttoDB = array_filter($porttoDB);
	}
	...
	{	
		for ($i=0; $i < count($porttoDB); $i++)
		{
			$query_add = "INSERT INTO `block`.`block_port` (`name` , `port` , `time`,`day`,`file_name`,`block_date_time`) VALUES ('".$_POST["name_add"]."','".$porttoDB[$i]."','".$timetoDB."','".$days."','".$filenametoDB."','".$blockdatetoDB."')";
